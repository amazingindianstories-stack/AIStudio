...
function item(patch: Partial<GenerationItem> = {}): GenerationItem {
  return {
    id: UUID_A,
    kind: "image",
    status: "succeeded",
...
function scope(patch: Partial<FeedScope> = {}): FeedScope {
...
test("malformed cursors decode to undefined rather than a NaN predicate", () => {
  ...
  for (const bad of ["", "abc", ".", "123.", "notanumber." + UUID_A, "123.not-a-uuid", null, undefined]) {
    assert.equal(decodeCursor(bad as string), undefined, `expected ${JSON.stringify(bad)} to be rejected`);
  }
});
