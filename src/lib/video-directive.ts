...
export type RefTagSyntax = "bracket" | "angle";

export interface VideoDirectiveInput {
  /** The user's prompt, already converted to the provider's tag syntax. Passed
   *  through verbatim from here on. */
  prompt: string;
  /** How many reference images are attached. 0 disables identity/style locks. */
  refCount: number;
  tagSyntax: RefTagSyntax;
}

function tagExample(syntax: RefTagSyntax): string {
...
export function hasCameraDirection(prompt: string): boolean {
 ...
 */
export function hasExplicitStyle(prompt: string): boolean {
...
function styleLock(refCount: number, promptNamesStyle: boolean): string {
 ...
 */
export function buildVideoDirective(input: VideoDirectiveInput): string {
