...
import { decodeCursor, encodeCursor, likePattern, type HistoryCursor } from "./store-db";
...
export interface AdminLogRow {
  ...
  costCents: number;
  userId: string | null;
  /** Truncated to PROMPT_PREVIEW_CHARS; see `promptTruncated`. */
  prompt: string;
...
export interface AdminLogPage {
  rows: AdminLogRow[];
  /** Rows matching the filter across the whole table, not just this page. */
  total: number;
  /** Cost of every matching row, so a filtered view can show a real subtotal. */
 ...
 *  erroring. */
export function parseAdminLogFilter(params: URLSearchParams): AdminLogFilter {
...
export function adminLogFilterToParams(filter: AdminLogFilter): URLSearchParams {
...
function conditions(filter: AdminLogFilter): SQL[] {
  const conds: SQL[] = [];
...
const previewColumns = {
  ...
  prompt: sql<string>`left(${generations.prompt}, ${PROMPT_PREVIEW_CHARS})`,
  promptTruncated: sql<boolean>`length(${generations.prompt}) > ${PROMPT_PREVIEW_CHARS}`,
...
export interface AdminCsvRow {
  ...
  costCents: number;
  userId: string | null;
  /** Untruncated — an export is the one place the whole prompt is wanted. */
  prompt: string;
  createdAt: number;
}
