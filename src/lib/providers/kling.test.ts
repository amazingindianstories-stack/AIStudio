...
const base: KlingImageInput = { model: "Kling Image 3.0", prompt: "a red bicycle" };
...
test("v1-only fidelity knobs are never sent", () => {
  ...
  const p = buildKlingPayload({ ...base, references: [ref] }) as unknown as Record<
    string,
    unknown
  >;
