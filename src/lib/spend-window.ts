...
export function spendLimitCents(env: EnvLike = process.env): number {
 ...
 */
export function bestOfMultiplier(env: EnvLike = process.env): number {
  return Math.min(4, Math.max(1, Number(env.FACE_BEST_OF) || 2));
}

export interface AdmissionInput {
  /** Estimated cents already committed inside the current window. */
  windowCents: number;
  /** Estimated cents this job would add. */
  jobCents: number;
  /** Budget for the window; 0 disables the gate. */
  limitCents: number;
  /** Whether anything else is running or recently finished in the window. */
  windowBusy: boolean;
}
 ...
 */
export function admits(i: AdmissionInput): boolean {
