...
export async function readGlobalLimit(key: string): Promise<number> {
...
export async function updateGlobalLimit(key: string, value: number): Promise<void> {
...
export async function readAllGlobalLimits(): Promise<Record<string, number>> {
  const db = await getDb();
...
export async function readUserLimits(userId: string): Promise<Record<string, number>> {
  const db = await getDb();
 ...
 *  per user to get it. */
export async function readAllUserLimits(): Promise<Record<string, Record<string, number>>> {
  const db = await getDb();
...
export async function readAllEffectiveLimits(
  ...
  const [globalLimits, userOverrides] = await Promise.all([
    readAllGlobalLimits(),
    userId ? readUserLimits(userId) : Promise.resolve({} as Record<string, number>),
  ]);
  const out: Record<string, number> = {};
  for (const def of LIMIT_DEFINITIONS) {
