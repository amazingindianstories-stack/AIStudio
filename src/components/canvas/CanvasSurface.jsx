...
const CREATE_TOOLS = ["rect", "ellipse", "triangle", "diamond", "text", "sticky", "frame"];
...
export function buildDefaultNode(tool, center) {
...
(function CanvasSurface({ toolLocked, onAfterSingleShotPlace, boardId, onTransientChange }, ref) {
  ...
  const [dropHighlightFrameId, setDropHighlightFrameId] = useState(null);
  const [dragGhost, setDragGhost] = useState(null);
  const [draftConnector, setDraftConnector] = useState(null);
  const [connectorHoverTargetId, setConnectorHoverTargetId] = useState(null);
  ...
  const toScreenLocal = useCallback((clientX, clientY) => {
  ...
  const onPointerDownHandle = (e, node, handle) => {
    (e.target ).setPointerCapture?.(e.pointerId);
    const screen = toScreenLocal(e.clientX, e.clientY);
    dragRef.current = {
      kind: "resize",
  ...
  const onConnectorHandlePointerDown = (e, node, anchor) => {
    e.stopPropagation();
    (e.target ).setPointerCapture?.(e.pointerId);
    const screen = toScreenLocal(e.clientX, e.clientY);
  ...
  const onDoubleClickBody = (e, node) => {
    e.stopPropagation();
  ...
  const onCommitText = (nodeId, text) => {
  ...
  const endDrag = () => {
    ...
    if (drag.kind === "connector") {
      ...
      const from = drag.connectorFromId
        ? { nodeId: drag.connectorFromId, anchor: drag.connectorFromAnchor ?? ("auto" ) }
        : (drag.connectorFromPoint ?? null);
      const to = connectorHoverTargetId
        ? { nodeId: connectorHoverTargetId, anchor: "auto"  }
  ...
  const cursorClass = isPanning
    ...
    ? "cursor-grabbing"
    : CREATE_TOOLS.includes(tool ) || tool === "connector"
    ? "cursor-crosshair"
    : "cursor-default";
  ...
  return (
    ...
    >
      <div className="absolute left-0 top-0 origin-top-left" style={{ transform: worldTransform }}>
        ...
        {dragGhost && CREATE_TOOLS.includes(tool ) && (
          <div
...
export async function uploadImageFile(file, boardId) {
  try {
    ...
    const res = await apiFetch(`/api/canvas-boards/${boardId}/upload`, {
      method: "POST",
