...
function HiggsfieldTokenCard() {
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = await apiFetch("/api/admin/set-token", {
        method: "POST",
...
function UsersTab({
  ...
  const deleteUser = async (user) => {
    setNotice(null);
    try {
      const response = await apiFetch(`/api/admin/users?id=${encodeURIComponent(user.id)}`, {
        method: "DELETE",
      });
      const json = await response.json().catch(() => ({}));
...
function AdminNoticeLine({ notice }) {
  return (
    <span
      role="status"
      aria-live="polite"
...
function LogsTab({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await apiFetch(`/api/admin/logs?${params}`, { cache: "no-store" });
        if (!res.ok) return;
...
function ActivityLog({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiFetch(`/api/admin/activity?${params}`, { cache: "no-store" });
        if (!res.ok) return;
        const page = await res.json();
        if (mine !== seq.current) return;
        setRows(page.rows);
