...
export function AudioTranscriptionModal({ open, onClose }) {
  ...
  const loadHistory = async () => {
    const res = await apiFetch("/api/history?kind=audio&limit=20", { cache: "no-store" });
    if (!res.ok) return;
  ...
  const transcribe = async () => {
    ...
    try {
      if (!/^audio\/(mpeg|mp3|wav|x-wav)$/i.test(file.type)) throw new Error("Choose an MP3 or WAV file.");
      ...
      const res = await apiFetch("/api/audio/transcribe", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ audioRef: ref, name: file.name }) });
      const data = await res.json();
  ...
  return (
    <div className="fixed inset-0 z-[80] grid place-items-center bg-black/70 p-4" role="dialog" aria-modal="true" aria-label="Audio transcription">
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-line bg-ink-900 shadow-2xl">
