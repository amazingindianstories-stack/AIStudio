...
export function AudioTranscriptionModal({ open, onClose, projectId }) {
  ...
  const selectFile = (candidate) => {
    ...
    const commonAudio = /^audio\/(mpeg|mp3|wav|x-wav|wave|ogg|webm|mp4|x-m4a|aac|flac)$/i;
    const byExtension = { mp3: "audio/mpeg", wav: "audio/wav", m4a: "audio/mp4", ogg: "audio/ogg", aac: "audio/aac", flac: "audio/flac", webm: "audio/webm" };
    const mime = candidate.type || byExtension[candidate.name.split(".").pop()?.toLowerCase()];
    if (!commonAudio.test(mime || "")) { setError("Choose MP3, WAV, M4A, OGG, AAC, FLAC or WebM audio."); return; }
    if (candidate.size > 15 * 1024 * 1024) { setError("Audio files must be 15 MB or smaller."); return; }
    setFile(candidate.type ? candidate : new File([candidate], candidate.name, { type: mime }));
    setError("");
  };
  ...
  return (
    <div ref={dialogRef} tabIndex={-1}
      ...
      onDrop={(event) => { event.preventDefault(); dragDepth.current = 0; setDragging(false); if (busy) return; const files = event.dataTransfer.files; if (files.length > 1) { setError("Drop one audio file at a time."); return; } selectFile(files[0]); }} onKeyDown={(event) => { if (event.key === "Escape") onClose(); }} className="fixed inset-0 z-[80] grid place-items-center bg-black/70 p-4" role="dialog" aria-modal="true" aria-label="Audio transcription">
      <div className="flex max-h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-line bg-ink-900 shadow-2xl">
        ...
        <div className="grid min-h-0 gap-5 overflow-y-auto p-5 md:grid-cols-[minmax(220px,0.8fr)_minmax(300px,1.3fr)_minmax(220px,0.9fr)]">
          <div><input ref={inputRef} type="file" accept="audio/*" hidden onChange={(e) => { selectFile(e.target.files?.[0]); e.target.value = ""; }} /><button type="button" disabled={busy} aria-label="Choose or drop audio file" onClick={() => inputRef.current?.click()} className={`flex min-h-36 w-full flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed text-sm text-white/65 hover:border-brand/60 disabled:opacity-50 ${dragging ? "border-brand bg-brand/10" : "border-line bg-ink-800"}`}><Upload className="h-6 w-6" />{file ? <span className="max-w-full truncate px-4 text-brand">{file.name}</span> : <span>{dragging ? "Drop your audio here" : "Drag and drop audio here, or click to browse"}</span>}</button>{file && <button onClick={transcribe} disabled={busy} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-brand px-4 py-2.5 text-sm font-semibold text-ink-900 disabled:opacity-50">{busy ? <><Loader2 className="h-4 w-4 animate-spin" /> Transcribing…</> : "Transcribe audio"}</button>}{error && <p role="alert" className="mt-3 rounded-lg bg-red-500/10 p-3 text-xs text-red-200">{error}</p>}<p className="mt-4 text-xs leading-5 text-white/45">Gemini 2.5 Flash analyzes speech, music, ambience and sound effects. Transcripts are saved in your history for reuse in future Seedance prompts.</p></div>
