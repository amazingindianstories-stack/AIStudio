...
| SEC-05 | Higgsfield refresh has no distributed lock | P1 | in_progress | 2026-08-29 | Codex | Refresh coordination is deployed and its PostgreSQL lease test passes. A bounded seven-day production-log query found no naturally occurring refresh event, so the one-rotation exit gate remains unproven; no refresh was forced. |
...
| COST-04 | Media delivery can silently proxy bytes | P1 | in_progress | 2026-08-29 | Codex | The deployed Admin Status check is ready, but no authenticated browser session was available in this run, so no direct-read result was recorded. Exit remains one authenticated production `ok` result without proxy fallback. |
...
| REL-07 | Repeated poll errors can leave a row running forever | P2 | monitoring | 2026-08-29 | Codex | The persisted terminal Omni marker remains visible, but a bounded seven-day `/api/generate/video/status` query hit its 100-result cap with HTTP 502 responses. The recurring 5xx exit gate therefore fails and this row remains monitoring. |
...
| VER-05 | Bundled ffmpeg is unverified on Vercel | P2 | in_progress | 2026-08-29 | Codex | The local executable check passes, but no authenticated production Admin Status session was available; target-runtime status remains unrecorded. |
| VER-06 | GCS signing under WIF is unconfirmed | P1 | in_progress | 2026-08-29 | Codex | The deployed one-byte real-object check is ready, but no authenticated production Admin Status session was available; an `ok` WIF direct-read result remains required. |
| VER-07 | Thumbnail warm-up was never run | P2 | open | 2026-08-29 | Codex | Vercel correctly redacts the sensitive WIF/storage values from environment exports, and gcloud requires interactive reauthentication. The dry run could not authenticate, so no apply run was attempted. |
| VER-08 | Kling 2K reference rule lacks controlled probe | P2 | open | 2026-08-29 | Codex | Both Vercel and local environment exports contain scrubbed Kling credentials, so the free validation-only probe could not authenticate; capability metadata was not changed. |
| VER-09 | Seedance continuation relies on third-party evidence | P2 | open | 2026-08-18 | Unassigned | Run the bounded billed probe. |
| VER-10 | Kling seed support is unconfirmed | P3 | open | 2026-08-29 | Codex | The free probe was attempted but could not authenticate with the scrubbed local/production exports. `supportsSeed` remains off for Kling. |
...
| ARCH-03 | Concurrency cap is global, not per user | P1 | in_progress | 2026-08-29 | Codex | Fresh-PostgreSQL fairness/override/tie coverage passes and the implementation is deployed. Two authenticated production user sessions were unavailable, so the required smoke fixture was not created. |
| ARCH-04 | Capabilities and prices key on display-name regexes | P2 | in_progress | 2026-08-29 | Codex | The explicit registry is deployed and all local routing/pricing tests pass. The free live Kling validation paths could not authenticate with scrubbed credentials, so production provider-route closure remains pending. |
...
| QUAL-03 | Flagged-generation signal has no consumer | P3 | in_progress | 2026-08-29 | Codex | Fresh-PostgreSQL coverage confirms flagged filtering and reason/judge evidence. No authenticated production admin session was available to create/review/export a self-cleaning fixture, so the representative RFC 4180 export gate remains pending. |
