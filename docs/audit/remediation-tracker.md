...
|---|---|---|---|---|---|
| SEC-01 | monitoring | AWS administrator + Codex | 2026-09-04 | Vercel Production and Preview use the verified exact-bucket read-only identity. Both old `vercel-s3-access` keys are inactive after IAM showed only historical S3 use; post-deactivation production storage and direct media checks pass. | Keep both keys recoverable through the observation window, then delete the legacy keys and temporary migration identity after the final gates pass. |
...
## Django cutover launch gates

The release cannot launch while any of these gates lacks deployed evidence:
`MIG-01`, `MIG-02`, `MIG-03`, `MIG-07`, `DRIFT-02`, `DRIFT-03`,
`DRIFT-05`, `VER-01`, `REL-01`, and `VER-04`. The signable evidence record is
`docs/cutover-go-no-go.md`. `SEC-01`, `COST-01`, and `MIG-06` remain monitoring
through 2026-09-10; their final checks cannot be satisfied early.
...
| SEC-01 | Exposed AWS access key is unrotated | P0 | monitoring | 2026-09-04 | AWS administrator + Codex | Read-only replacement is live in Vercel Production and Preview; both legacy full-access keys are inactive and retained only for reversible rollback through the seven-day observation window. |
...
| SEC-07 | No Content-Security-Policy | P2 | in_progress | 2026-09-05 | Codex | The Vite production build now rejects a missing/non-origin API URL and injects a CSP limited to self, the exact Railway API origin, and `storage.googleapis.com`; the former broad `https:` sources are gone. Closure requires deployed preview browser verification against the assigned origins. |
...
| COST-03 | No CDN in front of GCS | P1 | deferred | 2026-09-05 | Platform owner | Direct signed GCS delivery is the launch architecture. Cloud CDN is a post-launch project and does not block this cutover. |
...
| COST-07 | Pricing rows contain unverified placeholders | P2 | deferred | 2026-09-05 | Finance + platform owner | Invoice reconciliation is formally post-launch; provider probes remain capped and separately recorded in the go/no-go package. |
| REL-01 | Dead depth worker strands a running job | P1 | in_progress | 2026-09-05 | GPU operator + Codex | **Launch gate:** preview must show a healthy heartbeat plus deliberate claimed-job termination, old-claim fencing, stale recovery, resumption, terminal media read, and zero-residue cleanup. Any failure blocks launch. |
...
| MIG-01 | Django port is not cut over | P1 | in_progress | 2026-09-05 | Release commander + Codex | **Launch gate:** the release commit, isolated Railway/Vercel preview, complete workflow acceptance, final production read-only checks, and signed go/no-go package must pass before Django receives production traffic. |
| MIG-02 | Live routes are missing from Django | P1 | in_progress | 2026-09-05 | Codex | **Launch gate:** all retained API paths have Django counterparts and a method-inventory guard; deployed response/workflow parity must pass before traffic moves. |
| MIG-03 | Live tables are missing Django models | P1 | in_progress | 2026-09-05 | Database operator + Codex | **Launch gate:** models/adoption migrations exist, but read-only live catalog preflight and approved schema adoption remain mandatory. No production migration runs during preparation. |
| MIG-04 | Cloud SQL is staged but not live | P2 | deferred | 2026-09-05 | Platform owner | Railway PostgreSQL is the launch database. Cloud SQL is formally post-launch and is not part of this cutover. |
| MIG-05 | Cloud CDN is not provisioned | P1 | deferred | 2026-09-05 | Platform owner | Direct signed GCS delivery is the launch media path. Cloud CDN is formally post-launch. |
| MIG-06 | Rollback credentials were never removed | P1 | monitoring | 2026-09-03 | Operator + Codex | Zero-gap, restore, rotated-credential, Production/Preview deploy, and direct-media gates pass. Keep fallback and AWS variables through the seven-day window ending 2026-09-10. |
| MIG-07 | Railway Django service does not exist | P2 | in_progress | 2026-09-05 | Railway operator + Codex | **Launch gate:** API and both cron definitions exist; isolated preview services must pass, and production services must be configured but paused until the approved launch sequence. |
...
| MIG-09 | Unreachable legacy agent routes remain in Django | P3 | resolved | 2026-09-05 | Codex | The restored URL configuration exposes only the current conversation route family; the obsolete image/video/story agent routes remain absent. |
...
| DRIFT-02 | Django lacks video best-of-N pipeline | P2 | in_progress | 2026-09-05 | Codex | **Launch gate:** candidate submission, partial acceptance, cost proration, judging, and terminal settlement are implemented; deployed preview parity and cleanup remain mandatory. |
| DRIFT-03 | Django lacks server-side frame extraction | P2 | in_progress | 2026-09-05 | Codex | **Launch gate:** Django uses packaged ffmpeg for best-of-N frame extraction; the exact Railway runtime must pass preview verification. |
...
| DRIFT-05 | Django GCS authentication differs and is unverified | P2 | in_progress | 2026-09-05 | GCS operator + Codex | **Launch gate:** dedicated preview credentials must pass signed upload/read/range/thumbnail/protected-key/delete tests with direct delivery and zero residue. |
| DRIFT-06 | Django media reads lack abort propagation | P2 | deferred | 2026-09-05 | Backend owner | Railway's persistent sync-worker path is bounded by upstream timeouts; deeper disconnect cancellation is post-launch. Direct signed delivery prevents normal media bytes from traversing Django. |
...
| VER-01 | Django generation providers lack live calls | P1 | in_progress | 2026-09-05 | Provider-spend observer + Codex | **Launch gate:** mocked coverage passes; the separately approved preview probes must keep combined spend below US$2. A core-provider failure blocks launch and an optional failure disables that capability. |
| VER-02 | Django Higgsfield MCP path is unexercised | P2 | in_progress | 2026-09-05 | Codex | The MCP path and distributed refresh lease are restored; preview authentication and a billed request remain unapproved. |
| VER-03 | Django agent path lacks a live Gemini turn | P3 | in_progress | 2026-09-05 | Codex | Current conversation routes are restored with mocked coverage. A live Gemini turn requires separate billed-probe approval. |
| VER-04 | Depth worker lacks end-to-end production/GPU verification | P1 | in_progress | 2026-09-05 | GPU operator + Codex | **Launch gate:** healthy heartbeat; `vits`, `vitb`, and `vitl` end-to-end jobs; deliberate kill, claim fencing, stale recovery/resumption, media read, and zero-residue cleanup must all pass in preview. |
...
| VER-09 | Seedance continuation relies on third-party evidence | P2 | in_progress | 2026-09-05 | Provider-spend observer | Keep continuation disabled unless the approved minimum-duration preview probe passes within the combined US$2 ceiling. |
...
| VER-12 | Audio surcharge pricing lacks invoice verification | P3 | deferred | 2026-09-05 | Finance owner | Invoice reconciliation is post-launch. Seedance audio remains disabled while the evidence is inconclusive. |
| ARCH-01 | API responses use multiple envelopes | P2 | in_progress | 2026-09-04 | Codex | Django auth routes now use the canonical envelope and the client accepts canonical and legacy responses; migrate remaining route families after cutover stability. |
...
| DX-07 | Django history paths in `CLAUDE.md` are stale | P3 | resolved | 2026-09-05 | Codex | Maintained cutover documentation names the Vite SPA and Django API as the target runtime; the Next API is described only as the retained production/rollback implementation until cutover stability completes. |
...
| QUAL-02 | Eval harness has no fixtures or CI gate | P2 | deferred | 2026-09-05 | Quality owner | Eval-fixture expansion is formally post-launch; private face fixtures remain excluded from the public repository. |
...
- 2026-09-05: reconciled all 80 stable IDs against the Django + Vite cutover
  architecture. The ten Django/provider/depth items named above are explicit
  launch gates; `SEC-07` is implemented locally but awaits exact-origin preview
  verification; `SEC-01`, `COST-01`, and `MIG-06` remain observation-gated
  through 2026-09-10. Cloud SQL, Cloud CDN, code-size refactors, private eval-
  fixture expansion, and invoice reconciliation are formally deferred. Seedance
  audio, unsupported Kling seed/2K-reference combinations, and unverified
  continuation remain disabled. The register is 46 resolved, 18 in progress,
  4 monitoring, 10 deferred, and 2 open. No production or billed action ran.
...
- 2026-09-04: deactivated both legacy `vercel-s3-access` keys after the
  ...
  key's historical use only against S3 in `us-east-1`, the local runtime key
  matched neither legacy key, and no tracked file contained an AWS key
  literal. CloudTrail data-event coverage is not enabled, so this is recorded
  as the strongest available no-other-consumer evidence rather than a claim
  of exhaustive object-read telemetry. Both keys remain present and
  reversible until the observation window ends. Immediately afterward,
  authenticated production checks reported GCS reachable and direct media
  delivery HTTP 206, with zero production 5xxs in the surrounding 15-minute
  window. A fresh local reference scan could not start because GCP ADC now
  requires interactive reauthentication; the complete 7,983 checked / zero
  missing scan from 2026-09-03 remains the latest coverage evidence. `SEC-01`
  moves to monitoring; no fallback variable, AWS variable, key, user, or
  bucket was deleted. The register remains 57 resolved and 23 pending.
