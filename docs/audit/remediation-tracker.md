...
| REL-02 | Best-of-N holds all full-resolution candidates in memory | P1 | resolved | 2026-08-29 | Codex | After production deployment, the authenticated runtime audit exercised the real serial spool library, retained metadata only, and removed both the success and forced-failure temporary directories. A separate aggregate cleanup check found zero audit users, generations, limits, or marked fixtures. |
| REL-03 | Queue execution lacks an internal pre-timeout abort | P2 | resolved | 2026-08-29 | Codex | After production deployment, the authenticated runtime audit proved a short internal deadline persisted terminal failure before returning and left zero generation residue. A separate aggregate cleanup check found zero audit users, generations, limits, or marked fixtures. |
...
| MIG-04 | Cloud SQL is staged but not live | P2 | in_progress | 2026-08-29 | Codex | The post-deploy authenticated diagnostic confirms production still uses direct PostgreSQL. The reliability migration and PR #20 rollout completed safely on Railway; the separate Cloud SQL cutover remains deliberately held because its prior connector-certificate failure requires an extended preview soak before another production attempt. |
...
| ARCH-04 | Capabilities and prices key on display-name regexes | P2 | resolved | 2026-08-29 | Codex | The post-deploy authenticated validation-only diagnostic proved both registered wire models reached safe validation and that no provider task was created. Routing is now keyed by the registered wire model independently of the still-open resolution-capability mismatch. |
