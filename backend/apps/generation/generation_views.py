...
def _submit_video(base: dict) -> dict:
    ...
    if omni_provider.is_omni_model(model):
        ...
        def submit(candidate_seed):
            return seedance_provider.create_video_task(
        ...
        if requested > 1:
            ...
            for index in range(requested):
                try:
                    accepted.append(submit(seed + index if seed is not None else None))
                except Exception:
                    if not accepted:
                        raise
            task_id = accepted[0]
            ref_updates["candidateTaskIds"] = accepted[1:]
            ref_updates["costCents"] = round((base.get("costCents") or 0) * len(accepted) / requested)
        else:
            task_id = submit(seed)
...
def video_status(request):
    ...
    outcome = advance_video_status(item, source="browser")
    ...
    if outcome["kind"] == "raced":
        current = get_generation(item_id)
        if not current:
            return Response({"error": "Not found."}, status=404)
        return Response(row_to_item(current), headers={"Cache-Control": "no-store"})
    return Response(outcome["item"], headers={"Cache-Control": "no-store"})
...
def queue_execute(request):
    ...
    if base["kind"] == "video":
        try:
            requested_best_of = 1
            if (
                config.supports_video_best_of(model)
                and os.environ.get("VIDEO_BEST_OF")
                and base.get("referenceImages")
            ):
                try:
                    requested_best_of = min(3, max(2, int(os.environ["VIDEO_BEST_OF"])))
                except ValueError:
                    requested_best_of = 2
            if requested_best_of > 1:
                cost_cents *= requested_best_of
            running = _submit_video({
                **base,
                "seed": seed,
                "videoBestOf": requested_best_of,
    ...
    try:
        ...
        if mock.is_mock():
            ...
            render_size = requested_size
            best_of = bounded_best_of(os.environ.get("FACE_BEST_OF"), requested_size) if assembled.get("judgeFace") else 1
