...
class VideoStatusTransientErrorTests(TestCase):
    ...
    def test_transient_poll_error_does_not_report_failed(self):
        ...
        with patch(
            "apps.generation.video_reconciliation.seedance_provider.get_video_task",
        ...
        self.assertTrue(body.get("transientPollError"))
        self.assertEqual(body.get("pollErrorCount"), 1)
    ...
    def test_omni_input_block_is_persisted_once_and_returns_200(self):
        ...
        with patch(
            "apps.generation.video_reconciliation.omni_provider.get_omni_video_status",
            return_value={
                "status": "failed",
