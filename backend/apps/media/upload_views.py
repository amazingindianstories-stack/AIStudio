...
def presign_upload(request):
    ...
    if body.get("purpose") not in ("depth-input", "image-reference"):
    ...
    if not (content_type.startswith("video/") if body["purpose"] == "depth-input" else content_type in ("image/jpeg", "image/png", "image/webp", "image/tiff", "image/gif", "image/heic", "image/heif")):
        return Response({"error": f'depth-input does not accept content type "{content_type}".'}, status=400)
    key = f"uploads/{body['purpose']}/{request.user.id}-{uuid.uuid4()}"
    try:
