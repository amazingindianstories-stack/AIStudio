"""Stateless HMAC sessions shared at the token-format level with Next.js.

The cookies themselves are host-only: Vercel and Railway cannot read one
another's cookies. Matching the payload and signature preserves rollback
compatibility without introducing a parent-domain cookie.
...
def session_cookie_kwargs() -> dict:
    ...
    Cross-origin browser requests require SameSite=None and Secure. Omitting
    Domain is intentional so Railway owns its session cookie exclusively."""
    return {
        "httponly": True,
        "samesite": "None",
        "secure": True,
