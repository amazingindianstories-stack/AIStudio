"""Port of src/app/api/auth/{login,logout,me,password}/route.js. Django is
now a full session issuer (task #9) — see apps/common/session_auth.py's
module docstring for why the cookie contract must stay byte-identical to
auth.js for as long as Next.js can also still mint one.
"""
...
def login(request):
    ...
    response = Response({"user": _public_user(user)})
    response.set_cookie(
        "veevee_session", sign_session(str(user.id), user.auth_version), **session_cookie_kwargs()
    )
...
def logout(request):
    ...
    response.set_cookie("veevee_session", "", httponly=True, path="/", max_age=0)
    return response
...
def _clear_session(response):
    ...
    response.set_cookie(
        "veevee_session", "", httponly=True, samesite="Lax", secure=not settings.DEBUG, path="/", max_age=0
    )
    return response
...
def me(request):
    if request.method == "GET":
        user = request.user
        if not user:
            return _clear_session(Response({"user": None}))
        return Response({"user": _public_user(user)})

    # PATCH — profile update (name and/or avatar).
    session_user = request.user
    if not session_user:
        return _clear_session(Response({"error": "UNAUTHENTICATED"}, status=401))
    ...
    try:
        ...
        return Response({"user": _public_user(updated)})
    except InvalidAvatarError as e:
        delete_avatar_image(uploaded_avatar_url)
        return Response({"error": str(e)}, status=400)
    except Exception:
        delete_avatar_image(uploaded_avatar_url)
        return Response({"error": "Could not update the profile."}, status=500)
...
def password(request):
    ...
    if not updated_count:
        return _clear_session(Response({"error": "Your session changed. Sign in and try again."}, status=409))

    new_auth_version = account.auth_version + 1
    response = Response({"ok": True})
    response.set_cookie("veevee_session", sign_session(str(account.id), new_auth_version), **session_cookie_kwargs())
