"""Settings for the Railway Django API behind the Vercel Vite SPA."""
...
MIDDLEWARE = [
    ...
    "apps.common.origin.TrustedOriginMiddleware",
...
# AUTH_SECRET and the cookie name match the retained Next API during the
# rollback window. Cookies remain host-only and therefore do not move between
# Vercel and Railway.
...
SECURE_HSTS_SECONDS = env.int("SECURE_HSTS_SECONDS", default=0 if DEBUG else 63072000)
...
# The retired backend used dynamic managed=False model metadata and disabled
# DDL outside tests. Django is now the schema authority. For the legacy test
# flag only, build tables directly from current models so disposable SQLite
# smoke tests do not run the production adoption migrations.
if env.bool("DJANGO_TEST_MANAGED", default=False):
    MIGRATION_MODULES = {
        app: None
        for app in ("common", "media", "assets", "generation", "projects", "canvas", "agents", "admin_dashboard")
    }
