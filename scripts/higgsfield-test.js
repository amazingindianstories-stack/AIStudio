...
async function main() {
  ...
  const jobSetId = submit.id;
  console.log("  job-set:", jobSetId, "→ polling…");

  let jobSet = submit;
  const deadline = Date.now() + 4 * 60 * 1000; // 4 min
  while (Date.now() < deadline) {
    const statuses = (jobSet.jobs ?? []).map((j) => j.status);
    if (statuses.every((st) => !["queued", "in_progress"].includes(st))) {
      break;
    }
    await sleep(3000);
    jobSet = await api(`/v1/job-sets/${jobSetId}`);
    process.stdout.write(`  · ${(jobSet.jobs ?? []).map((j) => j.status).join(",")}\n`);
  }
