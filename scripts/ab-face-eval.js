...
async function judge(refFace, candidate) {
  const res = await fetch(
...
async function main() {
  ...
  const refPaths = row.reference_images ;
  ...
  async function finish() {
    for (const r of results) {
      if (r.file) {
        ...
        (r ).dims = `${meta.width}x${meta.height}`;
      }
    }
    ...
    for (const r of [...results].sort((a, b) => (b.identity ?? -2) - (a.identity ?? -2))) {
      console.log(
        r.error
          ...
          : `${r.name}: identity=${r.identity} quality=${r.quality} ${(r ).dims} ${Math.round((r.ms || 0) / 1000)}s — ${r.notes}`
      );
    }
    console.log("\nImages in:", OUT);
  }
