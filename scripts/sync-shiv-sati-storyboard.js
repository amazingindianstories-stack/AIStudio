...
import { CANVAS_STATE_VERSION, } from "../src/lib/canvas/types";
...
function plannedStillCount(row) {
...
async function listLatestStillPaths(row) {
...
async function prepareStills(rows, previous) {
  const sources = (
    await Promise.all(
...
async function main() {
  ...
  const rows = (JSON.parse(trackerBuffer.toString("utf8")) )
    .filter(isCanonical)
