...
import psutil
import requests
from depth_protocol import claim_payload, claimed_payload, heartbeat_payload
...
def _report_progress(job_id: str, claim_id: str | None, percent: int, message: str):
    try:
        _post(
            "/api/worker/depth/progress",
            claimed_payload(job_id, claim_id, percent=percent, message=message),
            timeout=10,
        )
...
def _complete_ok(job_id: str, claim_id: str | None, key: str, aspect_ratio: str | None):
    _post(
        "/api/worker/depth/complete",
        claimed_payload(job_id, claim_id, ok=True, key=key, aspectRatio=aspect_ratio),
    )


def _complete_failed(job_id: str, claim_id: str | None, error: str):
    try:
        _post(
            "/api/worker/depth/complete",
            claimed_payload(job_id, claim_id, ok=False, error=error[:2000]),
        )
    except Exception as e:  # noqa: BLE001 — best-effort; the server reaper owns abandoned claims
    ...
    with tempfile.TemporaryDirectory(prefix="depth-job-") as tmp:
        ...
        try:
            ...
            aspect_ratio = _run_depth(
                input_path, output_path, job.get("encoder") or "vitb",
                job.get("trackCharacters") is True, job_id, claim_id
            )

            _report_progress(job_id, claim_id, 92, "Requesting an upload slot")
            upload = _post(
                "/api/worker/depth/upload-url",
                claimed_payload(job_id, claim_id),
            )
            ...
            _complete_ok(job_id, claim_id, upload["key"], aspect_ratio)
...
def _claim_loop():
    while True:
        if _over_budget.is_set():
            ...
            continue
        try:
            resp = _post("/api/worker/depth/claim", claim_payload(WORKER_ID), timeout=15)
            job = resp.get("job")
